...
### OWED follow-up (deliberately NOT in Build 60 — keeps the FIX-2 diff to 4 lines)
- **€ glyph rides high on the Manuelle input.** `.encsrc-manual .cur { top:50% }` (CSS ~L761) is
  measured against the whole `.encsrc-manual` group (label **+** input), not the input alone, so the
  `top:50%` anchor lands near the boundary between the label and the field — the € sits at the field's
  top-left rather than vertically centred on it. It's small (13 px) and unobtrusive; reads fine.
  - **Fix when it's picked up (~1 CSS line, own build):** position `.cur` relative to the input alone —
    either wrap the input in its own relative box and anchor the € to that, or drop the label out of the
    positioning context so `top:50%` measures the field. **Not** the typing bug and out of scope here;
    Build 60 stays the clean 4-line FIX-2 so the cold-verify diff-scope gate is trivial to confirm.
